...
/* Changelog:
 *
 * IJB    2013-01-01 Added support for passing 'data-folder' via container for Wordpress themes
 *
...
//console.info('Loaded: jquery.cross-slide.js');
...
(function() {
	...
	$.fn.crossSlide = function(opts, plan, callback)
	{
		...
		(function(proceed) {
			...
			//console.log('FOLDER:%s', self.data('folder'));
			...
			function loop(i, img) {
				...
				img.src = folder + plan[i].src;
				console.log('IMG%d:%s --- %s', i, plan[i].src, img.src);
				if (i + 1 < plan.length)
					loop(i + 1, new Image());
			}
			loop(0, new Image());
			...
			if (! self.width() || ! self.height()) {
				console.info("Width:%s Height:%s (%s,%s)", self.width(), self.height(), self_width, self_height);
			...
			for (var i = 0; i < plan.length; ++i) {
				...
				var folder = ''; //self.data('folder') + '/';
				var img, elm;
				elm = img = $(format('<img src="{0}"/>', folder + p.src));
				if (p.href)
